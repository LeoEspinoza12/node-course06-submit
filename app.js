...
/////////////////////////////////////////////////////////////////
// this is a setup for HTTPS requests for all kinds of 
// http request (POST, PUT, GET, DELETE, etc..) and then we 
// will redirect the request to HTTPS route
app.all('*', (req, res, next) => {
  // if the request is coming from a secure port, (we will know this)
  // from the req.secure method, then we will say next()
  if (req.secure){
    return next();
  }
    // otherwise, it is not coming from a secured port
    // so we will redirect the request to an insecure port
    // then we will add the 307 which means that the resource resides 
    // temporarily on a different uri and the user agent must not change the
    // request method if it performs and automatic redirection to that 
    // uri
    else {
